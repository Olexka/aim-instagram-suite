...
function contentTeam(input) {
    ...
    const result = {
        ...
        nextStep: input.format === 'carousel'
            ...
            : 'Используй результат как готовый бриф для продакшена или передай в профильный AIM-инструмент.',
    mode: zod_1.z.enum(['setup', 'post', 'carousel', 'reels', 'vk', 'audit', 'content']).default('setup')
        .describe('setup=команда и кнопки; content=полный цикл; post/carousel/reels/vk/audit=узкий режим'),
    topic: zod_1.z.string().optional().describe('Тема, идея или исходный тезис'),
    sergeyContext: zod_1.z.string().optional().describe('Вайб Сергея: миссия, позиция, опыт, запреты, словарь, аудитория'),
    platform: zod_1.z.enum(['instagram', 'vk', 'both']).default('both'),
    goal: zod_1.z.enum(['subscribers', 'reach', 'saves', 'shares', 'trust', 'sales']).default('subscribers'),
    format: zod_1.z.enum(['post', 'carousel', 'reels', 'vk_post', 'all']).default('all'),
});
